...
`collect-host` 和 `collect-hardware`（`hardware-observe`）均保留计算力矩超限
检测和诊断安全事件，但不逐条打印力矩告警，结束后也不追加力矩告警汇总。
两种测算的耗时均不包含同步力矩告警输出的开销；正式实机和 Sim2Sim 的
力矩告警打印保持开启，因此测算结果不能作为开启告警输出时耗时的保证。

#### 第二阶段：吊装硬件观察

吊装测算同样关闭逐条力矩告警打印，保留超限检测、诊断安全事件及其它错误
输出。该设置只减少测算中的同步日志开销，不改变失能保活或失败退出处理。
...
每份 profiler 报告使用 schema v4，记录完整源码提交、主机节点/系统/内核/架构、
...
schema v3 缺少统一采样截止证据，结束年龄可能包含线程退出和统计排序耗时；
v1～v3 均须使用新部署包重新采集主机及吊装报告，禁止修改版本号后混用。

硬件报告的 `sampling_window` 使用 `steady` 时钟，语义版本为 1，记录起止时间、
窗口时长及是否已关闭；各源额外记录 `*_last_sample_offset_us`（相对窗口起点）。
最终策略采样结束或首次测算失败时，先关闭统一统计入口，等待已进入的短记录事务
完成并固定截止时间。左右电机同次反馈、单次 IMU 的原始/可信/配对证据不会被截止
拆开；后续回调不再进入统计。随后停止采样线程，在释放采样锁后计算分位数。
每个策略的推理分位数也延至传感器采集结束后计算，各策略仍使用有界样本缓存。
策略切换期间的已有反馈间隔继续保留，不通过删样本降低候选。

`final_age_us = sampling_window.duration_us - *_last_sample_offset_us`，
不计线程退出和排序耗时。分析器同时校验首末采样时间、完整 gap 均值乘计数与
统一窗口的一致性（绝对容差 1 微秒），缺失或矛盾的证据会拒绝分析。没有来源的
字段保持零值并标记未出现，不能冒充有效采样。独立 5 ms 失能保活不使用采样门锁，
在停止采样和统计阶段继续运行，最终失能发送及写失败仍纳入原有退出检查。
该修复不调整安全上限或 `base.yaml`；新报告的真实结束年龄仍参与超时候选计算。
