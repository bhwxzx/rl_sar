...
LW-077 起，profiler 报告使用 schema v4：在最终采样结束（或失败收尾）时统一冻结
传感器统计，再停止线程和计算分位数。`final_age_us` 是统一截止时间减去末次采样
时间，不包含退出和排序耗时；报告保留 `sampling_window` 和各源
`*_last_sample_offset_us` 供核验。独立失能保活覆盖统计阶段。
更新部署包后需重新采集主机和吊装报告；schema v1～v3 不能与新报告混用，
也不能仅删除旧报告的结束年龄来生成候选。首次反馈延迟、运行间隔和真实结束年龄
仍参与原有分析，四项 `MAX_SAFE_*` 上限仍需现场安全评审。
